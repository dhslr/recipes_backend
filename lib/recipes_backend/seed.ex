defmodule RecipesBackend.Seed do
  ...
  defp load_app do
    ...
    def call() do
      ...
      {:ok, currywurst} = Recipes.create_recipe(%{title: "Title", kcal: 2001, servings: 2, description: """
          1
        ...
        {:ok, _} = Recipes.create_ingredient(%{description: "Viel", food_id: ketchup.id, recipe_id: currywurst.id})
        {:ok, _} = Recipes.create_ingredient(%{quantity: 500, description: "Gramm", food_id: pommes.id, recipe_id: currywurst.id})
        {:ok, _} = Recipes.create_ingredient(%{quantity: 1, description: "Stück", food_id: wurst.id, recipe_id: currywurst.id})
        {:ok, _} = Recipes.create_ingredient(%{food_id: curry.id, recipe_id: currywurst.id})

        {:ok, _} = Accounts.register_user(%{email: "admin@dhslr.de", password: "supers3cret!", is_admin: true})
